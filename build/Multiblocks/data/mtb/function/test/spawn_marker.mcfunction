...
summon interaction ~ ~ ~ {Tags: ["test", "INIT"]}
tag @e[distance=0..1, type=marker, tag=rotor, tag=INIT] remove INIT
execute as @e[distance=0..0.1, type=marker, tag=test, tag=INIT] at @s rotated as @s run function mtb:test/spawn_marker/nested_execute_0
